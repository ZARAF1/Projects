...
let betterWords= storyWords.filter(element=>{
    return !unnecessaryWords.includes(element)// condition returns all elements that are not
    // included in the unnecessaryWords array
//    if unnecessaryWords does not include the current word,
//       you can return that word to the betterWords array.
})

console.log(betterWords.length)

// compare smaller array to bigger array to find matches not the opposite
let overWordCount = overusedWords.forEach(overWord=>{
    let count=0;
