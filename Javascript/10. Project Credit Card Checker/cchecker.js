...
const validateCred = (arr) => {
    ...
    if (i%2===0){
            doubledArr.push(reversedArr[i]*2)
        }
        else if (i%2===1){
            doubledArr.push(reversedArr[i])
        }
    }
    ...
    for (let i=0;i<doubledArr.length;i++){
        if (doubledArr[i]<=9){
            subtractedArr.push(doubledArr[i])
        }
        else if (doubledArr[i]>9){
            subtractedArr.push(doubledArr[i]-9)
        }
    }
    ...
    let finalSum=arrSum+checkDigit
